...
// Emojis dinâmicos para a raridade
...
export function buildShopEmbed(char: FullCharacter, category?: string): EmbedBuilder {
  ...
  // Puxa todos os itens da categoria que podem ser comprados (Preço maior que 0)
  const items = Object.values(ITEMS).filter(i => (i.type === category || i.slot === category) && i.price > 0);
  ...
  const lines = items.slice(0, 15).map(i =>
    `${RARITY_EMOJI[i.rarity.toLowerCase()] || RARITY_EMOJI['common']} ${i.emoji} **${i.name}** — 💰 **${i.price.toLocaleString('pt-BR')} G**\n` +
    `> *${i.description}*`
  ).join('\n\n');
...
export function buildShopItemSelect(char: FullCharacter, category: string): ActionRowBuilder<StringSelectMenuBuilder> | null {
  // Puxa todos os itens da categoria que podem ser comprados
  const items = Object.values(ITEMS).filter(i => (i.type === category || i.slot === category) && i.price > 0);
