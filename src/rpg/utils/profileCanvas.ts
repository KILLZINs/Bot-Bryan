...
export async function generateProfileCard(char: any, stats: any, avatarUrlInput?: string): Promise<Buffer> {
  ...
  // ==========================================
  // FUNDO DARK FANTASY (O ÉPICO COMEÇA AQUI)
  // ==========================================
  ...
  ctx.fillText(`NÍVEL ${level} — 🗡️ ${className.toUpperCase()}`, 175, headerY + 85);

  ctx.fillStyle = '#b5bac1';
  ctx.font = `14px ${GLOBAL_FONT}`;
  ctx.fillText(`🗺️ Localização: ${locationName}   |   💰 Ouro: ${gold.toLocaleString('pt-BR')} G`, 175, headerY + 115);
  ...
  const sy = panelY + 80;
  // Simulando status base apenas pro visual ficar exato como pedido
